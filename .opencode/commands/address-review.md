...
Read the full PR conversation, current HEAD SHA, base branch, implementation ticket, parent spec, relevant ADRs, and the latest Standards, Spec, and final acceptance comments.
...
For each clearly valid pending finding that is within the ticket's accepted scope, make the smallest correction that preserves the established architecture. Use TDD when behavior changes. Do not broaden the ticket.
