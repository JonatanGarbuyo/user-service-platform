...
Publish the final report as a top-level comment on that pull request via `gh pr comment`. Begin with `## Standards review — MiMo-V2.5` and include `Reviewed HEAD: <sha>`. If publication fails, preserve the complete report in your response and report the publication error.
