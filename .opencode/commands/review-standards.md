---
description: Run the Standards axis of project code review and publish it to the current pull request
agent: reviewer-standards
subagent: true
---

Read `AGENTS.md`, `CONTEXT.md`, `docs/agents/opencode.md`, relevant ADRs, and the repository-pinned `code-review` and `better-typescript` skills.
...
Run only the Standards axis: documented repository standards plus the skills' smell/technique baselines. Use `better-typescript` when reviewing TypeScript type design, but treat its technique preferences as judgement-call maintainability findings unless a documented project standard, accepted ADR, or ticket requirement makes them mandatory. Do not block solely because a different valid TypeScript technique could have been used. Report concrete findings with file/hunk evidence. Do not modify code.
